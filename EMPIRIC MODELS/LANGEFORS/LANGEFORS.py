...
# SOLVING THE USBM FORMULA WE GET A MODEL OF FORM: log(y)=log(K)+(B*log(sqrt(X2/(X1**(2/3)))))
# DEFINING A FUNCTION TO TRANSFORM THE DATA INTO DESIRE FORMAT
def log_transform(X,y):
    X_transform=np.log(np.sqrt(X[:,1]/(X[:,0]**(2/3))))
...
def predict_LANGEFORS( X , K , B ):
    return K*(((X[:,1]/(X[:,0]**(2/3)))**0.5)**(B))

pred_train=predict_LANGEFORS(X_train,K,B)
pred_test=predict_LANGEFORS(X_test,K,B)
